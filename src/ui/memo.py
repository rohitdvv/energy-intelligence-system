...
def _pdf(text: str) -> str:
    """Sanitise text to latin-1 safe characters for fpdf2 core fonts."""
    for char, sub in _PDF_SUBS:
...
def _generate_pdf_bytes(debate_result: dict[str, Any]) -> bytes:
    ...
    pdf.set_xy(10, 5)
    pdf.cell(0, 12, _pdf("ENERGY INTELLIGENCE SYSTEM -- INVESTMENT DEAL MEMO"), ln=True)
    ...
    pdf.cell(0, 12, _pdf(f"  VERDICT: {verdict_val}  |  CONVICTION: {conviction_val}"), fill=True, ln=True)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(4)
    ...
    def _section(title: str, body: str) -> None:
        ...
        pdf.set_fill_color(10, 20, 40)
        pdf.set_text_color(255, 255, 255)
        pdf.cell(0, 8, _pdf(f"  {title}"), fill=True, ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Helvetica", "", 9)
        pdf.ln(2)
        ...
        pdf.multi_cell(0, 5, _pdf(clean_body.strip()))
        pdf.ln(4)
    ...
    x = pdf.get_x()
    y = pdf.get_y()
    pdf.multi_cell(95, 5, _pdf(f"  {top_opp_val}"), border=1)
    h1 = pdf.get_y() - y
    pdf.set_xy(x + 95, y)
    pdf.multi_cell(95, 5, _pdf(f"  {top_risk_val}"), border=1)
    h2 = pdf.get_y() - y
